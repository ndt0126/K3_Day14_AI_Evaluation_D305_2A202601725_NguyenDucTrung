...
class LLMJudge:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        """
        score_sets = [entry.get("scores", {}) for entry in scores_batch]
        values = [float(value) for scores in score_sets for value in scores.values()]
        average = sum(values) / len(values) if values else 0.0

        # Comparative-judging callers may attach first/second answer scores.
        first_scores = [entry.get("first_score") for entry in scores_batch if entry.get("first_score") is not None]
        second_scores = [entry.get("second_score") for entry in scores_batch if entry.get("second_score") is not None]
        positional_bias = bool(first_scores and second_scores and sum(first_scores) / len(first_scores) > sum(second_scores) / len(second_scores))
        return {
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            result = evaluator.run_full_eval(
                agent_fn(pair.question), pair.question, pair.context or "", pair.expected_answer,
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        regressions = [metric for metric in metrics if baseline_averages[metric] - new_averages[metric] > 0.05]
        report = {f"new_avg_{metric}": new_averages[metric] for metric in metrics}
        report.update({f"baseline_avg_{metric}": baseline_averages[metric] for metric in metrics})
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            suggestion = suggestions[index - 1] if index <= len(suggestions) else "Investigate and prioritize a targeted fix"
            rows.append(f"| {failure_id} | {failure_type} | {self.find_root_cause(failure)} | {suggestion} | Open |")
        return "\n".join(rows)
    ...
    def generate_improvement_suggestions(
        ...
        if not failures:
            return []
        categories = {key.lower(): count for key, count in self.categorize_failures(failures).items()}
        suggestions: list[str] = []
        if categories.get("hallucination", 0):
            suggestions.append("Add grounding checks that remove or flag claims unsupported by retrieved context.")
        if categories.get("irrelevant", 0) or categories.get("off_topic", 0):
            suggestions.append("Clarify the prompt with intent-focused instructions and add routing examples for ambiguous questions.")
        if categories.get("incomplete", 0) or categories.get("low_completeness", 0):
            suggestions.append("Retrieve broader evidence and require the generator to cover all key conditions and exceptions.")
        generic = [
            "Review low-scoring traces, then add representative failures to the regression benchmark.",
            "Set CI quality gates for faithfulness, relevance, and completeness before deployment.",
            "Re-run the benchmark after each retrieval or prompt change and investigate metric drops over 0.05.",
        ]
